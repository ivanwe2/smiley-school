    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        ...
        <table style="width:100%; border-collapse:collapse;">
          <tr><td style="padding:8px; font-weight:bold;">Name</td><td>${escapeHtml(data.name)}</td></tr>
          <tr><td style="padding:8px; font-weight:bold;">Email</td><td>${escapeHtml(data.email)}</td></tr>
          ${data.phone ? `<tr><td style="padding:8px; font-weight:bold;">Phone</td><td>${escapeHtml(data.phone)}</td></tr>` : ""}
          <tr><td style="padding:8px; font-weight:bold;">Type</td><td>${escapeHtml(data.inquiryType)}</td></tr>
        </table>
        <h3 style="color: #1E3A5F;">Message</h3>
        <p style="background:#f5f5f5; padding:16px; border-radius:8px;">${escapeHtmlWithBreaks(data.message)}</p>
...
export async function sendContactConfirmation(
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        <h2 style="color: #0F1F3D;">Thank you, ${escapeHtml(name)}!</h2>
